...
(function (Scratch) {
  ...
  async function applyToTarget(runtime, target, transformSpec, gradDef) {
    ...
    const { canvas: srcCanvas, imageData, scale } = await rasteriseCostume(
      snapshot.costume, globalScaleOverride
    );
  ...
  // ── Per-target visual state (revision + costume sync) ────────────────────
  // targetVisualState : targetId → { costumeSkinId, visualRevision }
  const targetVisualState = new Map();
  ...
  function getTargetVisualState(target) {
    let state = targetVisualState.get(target.id);
    if (!state) {
      const costume = getCurrentCostume(target);
      state = {
        costumeSkinId: costume ? costume.skinId : null,
        visualRevision: 0,
      };
      targetVisualState.set(target.id, state);
    }
    return state;
  }
  ...
  function clearTargetEffectState(runtime, target) {
    ...
    const state = getTargetVisualState(target);
    state.visualRevision = (state.visualRevision + 1) | 0;

    const costume = getCurrentCostume(target);
    state.costumeSkinId = costume ? costume.skinId : null;
    return state;
  }

  function syncTargetVisualState(runtime, target) {
    const costume = getCurrentCostume(target);
    const costumeSkinId = costume ? costume.skinId : null;
    const state = getTargetVisualState(target);
    if (state.costumeSkinId !== costumeSkinId) {
      clearTargetEffectState(runtime, target);
    }
    return state;
  }

  function beginTargetMutation(runtime, target) {
    const state = syncTargetVisualState(runtime, target);
    const costume = getCurrentCostume(target);
    ...
    return {
      targetId: target.id,
      costume,
      costumeSkinId: costume.skinId,
      revision: state.visualRevision,
      rasterEpoch: globalRasterEpoch,
    };
  }

  function isSnapshotCurrent(snapshot) {
    const state = targetVisualState.get(snapshot.targetId);
    return !!state
      && state.visualRevision === snapshot.revision
      && state.costumeSkinId === snapshot.costumeSkinId
      && snapshot.rasterEpoch === globalRasterEpoch;
  }
  ...
  async function applyToLayer(runtime, target, layerName, transformSpec, gradDef) {
    ...
    const snapshot = beginTargetMutation(runtime, target);
    const { imageData: baseImageData } = await rasteriseCostume(snapshot.costume, globalScaleOverride);
    if (!isSnapshotCurrent(snapshot)) return;
  ...
  class CostumeColorFX {
    ...
    gradRemove(args, util) {
      ...
      if (state) {
        state.gradDef = null;
        const visual = syncTargetVisualState(this.runtime, target);
        visual.visualRevision = (visual.visualRevision + 1) | 0;
