...
(function (Scratch) {
  ...
  /**
   * Rasterise a costume to an offscreen canvas and return {canvas, ctx, imageData}.
   * Works for both SVG and bitmap costumes.
   */
  function rasteriseCostume(costume) {
  ...
  async function applyTransformToTarget(runtime, target, transformFn) {
    ...
    // Rasterise
    const { imageData } = await rasteriseCostume(costume);
    ...
    const skinId = renderer.createBitmapSkin(outCanvas, costume.bitmapResolution || 1);
