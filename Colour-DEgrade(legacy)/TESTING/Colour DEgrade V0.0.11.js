// Name: Colour DEgrade (Complete & Working)
// Description: Full dithering toolkit – Floyd‑Steinberg (serpentine, strength), ordered (monochrome, strength), threshold, pixelate, posterize, monochrome.
// License: MIT
...
(function (Scratch) {
  ...
  // ============================================================
  // DITHERING ALGORITHMS (working versions)
  // ============================================================

  // Floyd‑Steinberg with serpentine and strength
  function floydSteinberg(imageData, levels, serpentine, strength) {
    const data = imageData.data;
    const width = imageData.width;
    const height = imageData.height;
    const step = 255 / (levels - 1);
    const clamp = (v) => Math.min(255, Math.max(0, v));
    const errFactor = strength;
  ...
  // Ordered dither with matrix size, monochrome, strength
  function orderedDither(imageData, matrixSize, monochrome, strength) {
    const data = imageData.data;
    const width = imageData.width;
    const height = imageData.height;
    let matrix, dim;
  ...
  function thresholdDither(imageData, levels, monochrome) {
    const data = imageData.data;
    const step = 255 / (levels - 1);
  ...
  // Posterize (alias)
  const posterize = thresholdDither;
  ...
  // ============================================================
  // APPLY EFFECT (with cloning to avoid cache mutation)
  // ============================================================
  async function applyToTarget(runtime, target, effectFunc, ...args) {
    ...
    const canvas = getCanvas(cloned.width, cloned.height);
  ...
  class ColourDEgrade {
    ...
    async ditherFloydSteinberg(args, util) {
      ...
      await applyToTarget(this.runtime, target, floydSteinberg, levels, serpentine, strength);
    }
    ...
    async ditherOrdered(args, util) {
      ...
      const monochromeFlag = args.MONO === true || args.MONO === "true";
      const strength = Math.max(0, Math.min(100, Number(args.STRENGTH))) / 100;
      await applyToTarget(this.runtime, target, orderedDither, size, monochromeFlag, strength);
    }
    ...
    async ditherThreshold(args, util) {
      ...
      const levels = Math.max(2, Math.min(256, Number(args.LEVELS)));
      const monochromeFlag = args.MONO === true || args.MONO === "true";
      await applyToTarget(this.runtime, target, thresholdDither, levels, monochromeFlag);
    }
    ...
    async ditherPosterize(args, util) {
      const target = this._resolveTarget(args, util);
      const levels = Math.max(2, Math.min(256, Number(args.LEVELS)));
      await applyToTarget(this.runtime, target, posterize, levels, false);
    }
