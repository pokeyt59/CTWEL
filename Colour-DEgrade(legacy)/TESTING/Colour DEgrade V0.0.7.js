// Name: Colour DEgrade (Final)
// Description: Runtime dithering effects – Floyd‑Steinberg, ordered, threshold.
// License: MIT
...
(function (Scratch) {
  ...
  async function applyToTarget(runtime, target, effectFunc, ...args) {
    ...
    // Clone the ImageData to avoid mutating the cached original
    const clonedImageData = new ImageData(
      new Uint8ClampedArray(imageData.data),
      imageData.width,
      imageData.height
    );
    effectFunc(clonedImageData, ...args);
    const canvas = document.createElement("canvas");
    canvas.width = clonedImageData.width;
    canvas.height = clonedImageData.height;
    canvas.getContext("2d").putImageData(clonedImageData, 0, 0);
      ...
      return {
        ...
        blocks: [
          ...
          },
          {
            ...
            text: "current skin ID of [TARGET]",
            arguments: { TARGET: { type: Scratch.ArgumentType.STRING, defaultValue: "_myself_" } }
          }
        ]
      };
